              ...
              return (
                <div key={listing.id} className="card listing-card" onClick={() => setSelectedListing(listing)}>
                  <div className="lc-image">
                    ...
                    <span className={`badge badge-${listing.status}`} style={{ position: 'absolute', top: '0.5rem', right: '0.5rem', zIndex: 1 }}>{statusLabel}</span>
                  </div>
                  ...
                  <div className="lc-actions" onClick={e => e.stopPropagation()}>
                    <div className="lc-actions-left">
                      <Link to={`/seller/listings/edit/${listing.id}`} className="seller-btn seller-btn-secondary seller-btn-sm">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14" style={{ marginRight: '0.25rem' }}><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg> Edit
                      </Link>
                      {listing.status === 'paused' ? (
                        <button className="seller-btn seller-btn-secondary seller-btn-sm" onClick={() => handleUpdateStatus(listing.id, 'active')}>
                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14" style={{ marginRight: '0.25rem' }}><polygon points="5 3 19 12 5 21 5 3"/></svg> Resume
                        </button>
                      ) : (
                        <button className="seller-btn seller-btn-secondary seller-btn-sm" onClick={() => handleUpdateStatus(listing.id, 'paused')}>
                          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14" style={{ marginRight: '0.25rem' }}><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> Pause
                        </button>
                      )}
                    </div>
                    <div className="lc-menu">
                      <button className="seller-btn seller-btn-ghost seller-btn-sm lc-menu-btn" aria-label="Actions" onClick={(e) => {
                         e.stopPropagation();
                      ...
                      <div className="lc-dropdown" onClick={e => e.stopPropagation()}>
                        {listing.status !== 'sold_out' ? (
                          <button className="lc-dropdown-item" onClick={() => handleUpdateStatus(listing.id, 'sold_out')}>
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12"/></svg> Mark sold out
                          </button>
                        ) : (
                          <button className="lc-dropdown-item" onClick={() => handleUpdateStatus(listing.id, 'active')}>
